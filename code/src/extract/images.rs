...
pub struct ImageFigures {
    ...
    /// v1 schema (prompt file deleted, board:cleanup.remove_vlm_anthropic) -- LEGACY.
    /// `amount_due_before_date` is
    ...
    /// v2 schema (prompt file deleted, `finding.image05_root_cause`): unambiguous
    ...
    /// v3 schema (prompt file deleted, image_accuracy_plan.md §1/§2 witness gate): a
...
fn select_pending_or_scheduled(figures: &ImageFigures, event: &Event) -> Option<f64> {
    ...
    // v1 schema (LEGACY, prompt file deleted) -- a cutoff may come from the labeled
