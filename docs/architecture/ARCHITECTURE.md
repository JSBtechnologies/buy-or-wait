...
6. **Built with [atrium](https://github.com/nativelite/atrium), my own agentic harness.** atrium is "tmux for coding agents": a multi-agent terminal that hosts real CLI agents (here, Claude Code) in switchable, splittable panes. The whole solution was developed by an atrium **fleet** (`atrium.fleet.json`, kickoffs and rules in [`fleet/`](../../fleet/)), with me supervising and making every product ruling.
